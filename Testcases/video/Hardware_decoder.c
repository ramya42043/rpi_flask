...
typedef struct _CustomData {
	GstElement *pipeline,*demux,*source,*parse,*decoder,*videoconv,*audioconv,*audiosink,*videosink,*resample,*videobox;
...
gint start()
{
	...
	data.demux = gst_element_factory_make ("qtdemux", "video demuxer");
	if(!data.demux){
		g_print("Failed to create  video demuxer\n");
		return -1;
	}
	data.parse = gst_element_factory_make ("h264parse", "video parser");
	if(!data.demux){
		g_print("Failed to create  video parser\n");
		return -1;
	}
	data.decoder = gst_element_factory_make ("omxh264dec", "video decoder");
	if(!data.decoder){
	...
	g_object_set (G_OBJECT (data.source), "location", "./sample.mp4", NULL);
	...
	gst_bin_add_many (GST_BIN (data.pipeline),
			data.source,data.decoder,data.demux,data.parse,data.audiosink,data.videobox,data.videosink,data.resample,data.audioconv,data.videoconv,NULL);
	/* we link the elements together */
	gst_element_link (data.source, data.demux);
	gst_element_link (data.demux, data.parse);
...
//	gst_element_link (data.resample,data.audiosink);
	...
	g_signal_connect (data.demux, "pad-added", G_CALLBACK (on_pad_added), data.parse);

	g_signal_connect(data.parse,"pad-added",G_CALLBACK(on_pad_added),data.decoder);
