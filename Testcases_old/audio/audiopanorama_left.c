...
static gfloat panorama=0;
typedef struct _CustomData {
		GstElement *pipeline,*source,*vol,*decoder,*conv,*alsasink,*test,*panorama;
...
static int change_panorama(CustomData *data,gfloat i)
{
		gfloat lpanorama;
		panorama=i;
		g_object_set (G_OBJECT (data->panorama),"panorama",i,NULL);
		g_object_get(data->panorama,"panorama",&lpanorama,NULL);

		if(panorama==lpanorama)
			return 1;
		else
			return 0;
}
...
gint start()
{
		...
		data.panorama=gst_element_factory_make("audiopanorama","panorama");
		if(!data.panorama){
				g_print("failed to create panorama\n");
				return -1;
		}
		...
		gst_bin_add_many(GST_BIN(data.pipeline),data.source,data.decoder,data.panorama,data.vol,data.conv,data.alsasink,NULL);

		/** Linking all the elements**/
		gst_element_link(data.source,data.decoder);
		gst_element_link(data.decoder,data.conv);
		gst_element_link(data.conv,data.panorama);
		gst_element_link(data.panorama,data.alsasink);
		//gst_element_link(data.vol,data.alsasink);

		g_signal_connect(data.decoder, "pad-added", G_CALLBACK(on_pad_added),data.conv);
		g_signal_connect(data.conv, "pad-added", G_CALLBACK(on_pad_added),data.panorama);
		//g_signal_connect(data.panorama, "pad-added", G_CALLBACK(on_pad_added),data.vol);
		g_signal_connect(data.panorama,"pad-added",G_CALLBACK(on_pad_added),data.alsasink);
